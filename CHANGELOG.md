...
- [lib][api] Packet classes now store concrete variable and payload headers inline instead of heap-allocating header objects, while preserving existing public accessors.

### Added

- [lib][api] Added minimal `kmStd` thread aliases for project-used threading primitives and `this_thread::sleep_for` so custom thread includes only need to provide the required surface.
- [lib][api] Added `kmStd::chrono` wrapper with `CUSTOM_CHRONO_INCLUDE` support for SDK-used clock and duration primitives.
...
- [lib][api] Added custom allocator foundation with per-client allocator injection through `MqttClientOptions`.
