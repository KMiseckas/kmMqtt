...
MqttClientImpl::MqttClientImpl(const IMqttEnvironment *const env,
  ...
  m_socket->setOnConnectCallback(
      [this](bool success) { handleSocketConnectEvent(success); });
  m_socket->setOnDisconnectCallback(
      [this]() { handleSocketDisconnectEvent(); });
  m_socket->setOnRecvdCallback([this](ByteBuffer &&buffer) {
    handleSocketDataReceivedEvent(std::move(buffer));
  });
  m_socket->setOnErrorCallback(
      [this](std::uint16_t error) { handleSocketErrorEvent(error); });

  m_sendQueue.setSocket(m_socket);
  m_sendQueue.setOnPingSentCallback([this]() { handlePingSentEvent(); });
  m_sendQueue.setOnPubCompSentCallback(
      [this](std::uint16_t packetId) { handlePubCompSentEvent(packetId); });
  m_sendQueue.setOnPubRecSentCallback(
      [this](std::uint16_t packetId) { handlePubRecSentEvent(packetId); });
  m_sendQueue.setOnPubRelSentCallback(
      [this](std::uint16_t packetId) { handlePubRelSentEvent(packetId); });
  m_sendQueue.setOnDisconnectSentCallback(
      [this]() { handleDisconnectSentEvent(); });
}
...
ReqResult MqttClientImpl::connect(ConnectArgs &&args,
                                  ConnectAddress &&address) noexcept {
  {
    ...
    if (m_connectionStatus == ConnectionStatus::CONNECTED ||
      ...
      LogWarning("MqttClient", "Cannot connect() again while already connected "
                               "or attempting to connect.");
      return m_connectionStatus == ConnectionStatus::CONNECTED
                 ? ReqResult{ClientErrorCode::Already_Connected}
                 : ReqResult{ClientErrorCode::Already_Connecting};
    }

    if (address.primaryAddress.hostname().empty()) {
      LogError("MqttClient", "Cannot connect() without primary address.");
      return ReqResult{ClientErrorCode::Missing_Argument,
                       "Cannot connect() without primary address."};
    }

    if (args.clientId.empty()) {
      LogError("MqttClient", "Cannot connect() without client ID argument.");
      return ReqResult{ClientErrorCode::Missing_Argument,
                       "Cannot connect() without client ID argument."};
    }

    if (args.will != nullptr) {
      if (args.will->payload != nullptr && args.will->payload->size() <= 0) {
        LogError("MqttClient", "Payload size must be bigger than 0! Payload is "
                               "included in the will, but payload size is 0.");
        return ReqResult{ClientErrorCode::Invalid_Argument,
                         "Payload size must be bigger than 0! Payload is "
                         "included in the will, but payload size is 0."};
      }

      if (args.will->correlationData != nullptr &&
          args.will->correlationData->size() <= 0) {
        LogError(
            "MqttClient",
            "Correlatation data size must be bigger than 0! Correlatation data "
            "is included in the will, but Correlatation data size is 0.");
        return ReqResult{
      ...
      if (args.will->willTopic.empty()) {
        LogError("MqttClient", "Attempted to add a Will to the Connect packet, "
                               "but Will Topic has not been set!");
        return ReqResult{ClientErrorCode::Missing_Argument,
                         "Attempted to add a Will to the Connect packet, but "
                         "Will Topic has not been set!"};
      }
    }
    ...
    LogInfo("MqttClient",
            ...
            address.primaryAddress.port().c_str(), args.username.c_str(),
            args.clientId.c_str(), static_cast<std::uint8_t>(args.version),
    ...
    m_connectionInfo.receiveMaximumAsClient =
        m_connectionInfo.connectArgs.receiveMaximum == 0
            ? RECEIVE_MAXIMUM_DEFAULT
            : m_connectionInfo.connectArgs.receiveMaximum;

    LogInfo("MqttClient", "Socket attempting to connect.");

    const bool sConnectResult{
        m_socket->connect(m_connectionInfo.connectAddress.primaryAddress)};
...
ReqResult MqttClientImpl::publish(const char *topic, ByteBuffer &&payload,
  ...
  if (m_connectionStatus != ConnectionStatus::CONNECTED) {
    ...
    return ReqResult{ClientErrorCode::Not_Connected,
                     "Client not connected, cannot publish()!"};
  }

  if (options.topicAlias > m_connectionInfo.maxServerTopicAlias) {
    LogError(
        "MqttClient",
        "Topic alias exceeds `max server topic alias` received from broker.");
  }
  ...
  if (options.qos >= mqtt::Qos::QOS_1) {
    ...
    const auto error{
        m_connectionInfo.sessionState.addMessage(packetId, std::move(msgData))};
  ...
  LogInfo(
      "MqttClient",
      "Queued publish message for sending, Topic: %s, Packet ID: %d, QOS: %d",
      topic, packetId, static_cast<std::uint8_t>(options.qos));
...
ReqResult MqttClientImpl::subscribe(const std::vector<Topic> &topics,
  ...
  if (m_connectionStatus != ConnectionStatus::CONNECTED) {
    ...
    return ReqResult{ClientErrorCode::Not_Connected,
                     "Client not connected, cannot subscribe()!"};
  }

  if (topics.empty()) {
    LogError("MqttClient", "Cannot subscribe to an empty list of topics!");
    return ReqResult{ClientErrorCode::Invalid_Argument,
                     "Cannot subscribe to an empty list of topics!"};
  }

  if (!m_connectionInfo.subscribeIdentifiersSupported &&
      options.subscribeIdentifier.uint32Value() > 0) {
    LogError("MqttClient", "Subscribe Identifiers not supported by broker.");
    return ReqResult{ClientErrorCode::SubscribeIdentifiersNotSupported,
                     "Subscribe Identifiers not supported by broker."};
  }
  ...
  {
    LockGuard guard{m_mutex};
    m_connectionInfo.pendingSubscriptions.push_back(
  ...
  LogTrace("MqttClient", "Started subscribe: Subscribing to; %s",
           allTopicsToStr(topics).c_str());
  m_mqttMainThreadCondition.notify_all();
...
ReqResult MqttClientImpl::unSubscribe(const std::vector<Topic> &topics,
  ...
  if (m_connectionStatus != ConnectionStatus::CONNECTED) {
    ...
    return ReqResult{ClientErrorCode::Not_Connected,
                     "Client not connected, cannot unSubscribe()!"};
  }

  if (topics.empty()) {
    LogError("MqttClient", "Cannot unsubscribe with an empty list of topics!");
    return ReqResult{ClientErrorCode::Invalid_Argument,
                     "Cannot unsubscribe with an empty list of topics!"};
  }
  ...
  {
    LockGuard guard{m_mutex};
    m_connectionInfo.pendingUnSubscriptions.push_back(
  ...
  LogTrace("MqttClient", "Started unSubscribe: Unsubscribing from %s",
           allTopicsToStr(topics).c_str());
...
ReqResult MqttClientImpl::disconnect(DisconnectArgs &&args) noexcept {
  if (m_connectionStatus == ConnectionStatus::DISCONNECTED) {
    LogWarning("MqttClient",
               "Client already disconnected, cannot disconnect()!");
    return ReqResult{ClientErrorCode::Not_Connected,
                     "Client not connected, cannot disconnect()!"};
  }

  handleInternalDisconnect(
      args.willPublish ? DisconnectReasonCode::DISCONNECT_WITH_WILL_MESSAGE
...
ClientError MqttClientImpl::shutdownAsync() noexcept {
  ...
  if (!m_isRunningAsync.compare_exchange_strong(expectedVal, false)) {
    LogInfo(
        "MqttClient",
        "Client not running asynchronously, cannot shutdownAsync(). Ignoring.");
    return ClientErrorCode::No_Error;
  }
...
ClientError MqttClientImpl::shutdownCleanup() noexcept {
  ...
  m_pubCompletedEvent
      .removeAll(); // Not just for pubComp packet, but all publish packets;
...
ClientError MqttClientImpl::tick() noexcept {
  assert(m_clientOptions.getTickMode() == TickMode::SYNC &&
         "tick() should only be called when client is not configured to tick "
         "asynchronously.");

  if (m_clientOptions.getTickMode() == TickMode::ASYNC) {
    LogWarning("MqttClient",
               "Cannot call tick() when client is configured to tick "
               "asynchronously (tickAsync set as true in constructor).");
...
void MqttClientImpl::tickAsync() noexcept {
  assert(m_clientOptions.getTickMode() == TickMode::ASYNC &&
         "tickAsync() should only be called when client is configured to tick "
         "asynchronously.");

  if (m_isRunningAsync.exchange(true)) {
    LogWarning("MqttClient",
               "tickAsync() called while already ticking asynchronously.");
    return;
  }
...
ErrorEvent &MqttClientImpl::onErrorEvent() noexcept { return m_errorEvent; }
...
void MqttClientImpl::pubComp(std::uint16_t packetId, PubCompReasonCode code,
...
bool MqttClientImpl::tryStartBrokerRedirection(
    std::uint8_t failedConnectionReasonCode,
    const Properties &properties) noexcept {
  static_assert(
      static_cast<std::uint8_t>(ConnectReasonCode::SERVER_MOVED) ==
          static_cast<std::uint8_t>(DisconnectReasonCode::SERVER_MOVED),
      "ConnectReasonCode::SERVER_MOVED value no longer matches "
      "DisconnectReasonCode::SERVER_MOVED value: Required "
      "`tryBrokerRedirection()` logic to be amended.");
  static_assert(
      static_cast<std::uint8_t>(ConnectReasonCode::USE_ANOTHER_SERVER) ==
          static_cast<std::uint8_t>(DisconnectReasonCode::USE_ANOTHER_SERVER),
      "ConnectReasonCode::USE_ANOTHER_SERVER value no longer matches "
      "DisconnectReasonCode::USE_ANOTHER_SERVER value: Required "
      "`tryBrokerRedirection()` logic to be amended.");

  static constexpr std::uint8_t SERVER_MOVED_VAL =
      static_cast<std::uint8_t>(DisconnectReasonCode::SERVER_MOVED);
  static constexpr std::uint8_t USE_ANOTHER_SERVER_VAL =
      static_cast<std::uint8_t>(DisconnectReasonCode::USE_ANOTHER_SERVER);

  if (failedConnectionReasonCode == SERVER_MOVED_VAL ||
      failedConnectionReasonCode == USE_ANOTHER_SERVER_VAL) {
    const UTF8String *serverRef;
    const bool hasServerReference{
        properties.tryGetProperty(PropertyType::SERVER_REFERENCE, serverRef)};
    ...
    if (hasServerReference) {
      if (serverRef->stringSize() != 0) {
        ...
        if (failedConnectionReasonCode == SERVER_MOVED_VAL) {
          LogInfo("MqttClient",
                  "Disconnect packet contains a SERVER_REFERENCE for new "
                  "temporary server: ",
                  serverRef->getString().c_str());
        ...
        LogWarning(
            "MqttClient",
            "Disconnect packet contains an empty SERVER_REFERENCE property!");
      }
    }

    if (m_connectionInfo.reconnectAddress.tryCycleToNextPrimaryAddress()) {
      LogInfo("MqttClient",
              "Disconnect reason is %s but no SERVER_REFERENCE, will try "
              "reconnect using additionally provided addresses. ",
              failedConnectionReasonCode == SERVER_MOVED_VAL
                  ? "SERVER_MOVED"
                  : "USE_ANOTHER_SERVER");
...
void MqttClientImpl::reconnect() {
  DISPATCH_EVENT_TO_CONSUMER([&]() {
    m_reconnectEvent(
        {ReconnectionStatus::RECONNECTING, false, 0, ClientErrorCode::No_Error},
        ConnectAck{});
  });
...
void MqttClientImpl::handleInternalDisconnect(
    DisconnectReasonCode reason, const DisconnectArgs &args) noexcept {
  {
    LockGuard guard{m_mutex};

    LogInfo("MqttClient", "Starting internal disconnect: %s",
            args.disconnectReasonText.c_str());

    if (m_connectionStatus == ConnectionStatus::DISCONNECTED) {
      LogWarning("MqttClient", "Client not connected, cannot perform internal "
                               "disconnect. Ignoring.");
      return;
    }

    // Try gracefully disconnecting by sending Disconnect packet through send
    // queue.
    ...
    // Ignore graceful disconnection - bypass send queue and send packet ASAP.
    // Consider Disconnect done straight away.
    ...
    if (encodeResult.isSuccess()) {
      if (sendPacket(packet) != 0) {
        LogWarning("MqttClient",
                   "Failed to send disconnect packet during internal "
                   "disconnect. Skipping sending to broker.");
      }
    } else {
      LogWarning("MqttClient",
                 "Failed to encode disconnect packet during internal "
                 "disconnect. Skipping sending to broker.");
    }
    ...
    if (!m_socket->close()) {
      LogError("MqttClient",
               "Error closing socket properly during internal disconnect: %d",
  ...
  DISPATCH_EVENT_TO_CONSUMER([&, reasonCode = reason]() {
    m_disconnectEvent({reasonCode, false, false, ClientErrorCode::No_Error});
  });
}
...
void MqttClientImpl::handleExternalDisconnect(const Disconnect &packet) {
  {
    ...
    LogInfo("MqttClient",
            "Starting disconnect from received Disconnect packet with reason "
            "code: %d",
            static_cast<std::uint8_t>(packet.getVariableHeader().reasonCode));

    const UTF8String *reasonText;
    if (packet.getVariableHeader().properties.tryGetProperty(
            PropertyType::REASON_STRING, reasonText)) {
      LogDebug("MqttClient", "Disconnect Reason String: %s",
               reasonText->getString().c_str());
    }
    ...
    const bool brokerRedirectionStarted = tryStartBrokerRedirection(
        static_cast<std::uint8_t>(code), packet.getVariableHeader().properties);
  ...
  DISPATCH_EVENT_TO_CONSUMER(
      [&, reasonCode = packet.getVariableHeader().reasonCode]() {
        m_disconnectEvent({reasonCode, true, false, ClientErrorCode::No_Error});
      });
}
...
void MqttClientImpl::handleExternalDisconnect(int closeCode,
  ...
  DISPATCH_EVENT_TO_CONSUMER([&]() {
    m_disconnectEvent({DisconnectReasonCode::UNSPECIFIED_ERROR, true, false,
                       ClientErrorCode::Socket_Error});
  });
}
...
void MqttClientImpl::handleSocketConnectEvent(bool success) {
  if (success) {
    ...
    ConAckCallback conAckCallback{
        std::bind(&MqttClientImpl::handleReceivedConnectAcknowledge, this,
                  std::placeholders::_1)};
    DisconnectCallback disconnectCallback{
        std::bind(&MqttClientImpl::handleReceivedDisconnect, this,
                  std::placeholders::_1)};
    PubCallback pubCallback{std::bind(&MqttClientImpl::handleReceivedPublish,
                                      this, std::placeholders::_1)};
    PingRespCallback pingRespCallback{
        std::bind(&MqttClientImpl::handleReceivedPingResponse, this,
                  std::placeholders::_1)};
    PubAckCallback pubAckCallback{
        std::bind(&MqttClientImpl::handleReceivedPublishAck, this,
                  std::placeholders::_1)};
    PubRecCallback pubRecCallback{
        std::bind(&MqttClientImpl::handleReceivedPublishRec, this,
                  std::placeholders::_1)};
    PubRelCallback pubRelCallback{
        std::bind(&MqttClientImpl::handleReceivedPublishRel, this,
                  std::placeholders::_1)};
    PubCompCallback pubCompCallback{
        std::bind(&MqttClientImpl::handleReceivedPublishComp, this,
                  std::placeholders::_1)};
    SubAckCallback subAckCallback{
        std::bind(&MqttClientImpl::handleReceivedSubscribeAcknowledge, this,
                  std::placeholders::_1)};
    UnSubAckCallback unSubAckCallback{
        std::bind(&MqttClientImpl::handleReceivedUnSubscribeAcknowledge, this,
                  std::placeholders::_1)};
    ...
    LogTrace(
        "MqttClient",
        "Connect packet send request queued, to be resolved on next tick.");
  } else {
    LogError("MqttClient", "Socket failed to connect!");
    const bool wasReconnecting{m_connectionStatus ==
    ...
    if (wasReconnecting) {
      ...
      m_connectionStatus = ConnectionStatus::DISCONNECTED;
      DISPATCH_EVENT_TO_CONSUMER([&, p = ConnectAck{}]() {
        m_connectEvent({false, false, ClientErrorCode::Socket_Connect_Failed},
                       p);
      });
    }
  }
...
void MqttClientImpl::handleSocketDisconnectEvent() {
  handleExternalDisconnect(m_socket->getLastError(),
                           m_socket->getLastCloseReason());
}
...
void MqttClientImpl::handleSocketDataReceivedEvent(ByteBuffer &&buffer) {
  ...
  if (leftOver > 0) {
    ...
    m_leftOverBuffer.append(fullBuffer.bytes() + fullBuffer.size() - leftOver,
                            leftOver);
  }
}
...
void MqttClientImpl::handleSocketErrorEvent(int error) {
  ...
  if (m_connectionStatus == ConnectionStatus::CONNECTING ||
      m_connectionStatus == ConnectionStatus::RECONNECTING) {
    DISPATCH_EVENT_TO_CONSUMER([this, p = ConnectAck{}]() {
      m_connectEvent({false, false, ClientErrorCode::Socket_Connect_Failed}, p);
    });
    return;
  }
...
void MqttClientImpl::handlePubRecSentEvent(std::uint16_t packetId) {
  ...
  m_connectionInfo.sessionState.updateMessage(
      packetId, PublishMessageStatus::WaitingForPubRel);
}
...
void MqttClientImpl::handlePubRelSentEvent(std::uint16_t packetId) {
  ...
  m_connectionInfo.sessionState.updateMessage(
      packetId, PublishMessageStatus::WaitingForPubComp);
}
...
void MqttClientImpl::handleDisconnectSentEvent() {
  ...
  DISPATCH_EVENT_TO_CONSUMER([&]() {
    m_disconnectEvent(
        {m_gracefulDisconnectReason, false, true, ClientErrorCode::No_Error});
  });
}

void MqttClientImpl::handleReceivedConnectAcknowledge(ConnectAck &&packet) {
  // Connection counts as succeeded for any ConnectReasonCode < 128U so report
  // back as connection success, else start failure proccess.
  if (packet.getVariableHeader().reasonCode <
      ConnectReasonCode::UNSPECIFIED_ERROR) {
    ...
    if (packet.getVariableHeader().properties.tryGetProperty<std::uint16_t>(
            PropertyType::SERVER_KEEP_ALIVE, serverKeepAlive)) {
      m_connectionInfo.serverKeepAlive = *serverKeepAlive;
    } else {
      m_connectionInfo.serverKeepAlive =
          m_connectionInfo.connectArgs.keepAliveInSec;
    }
    ...
    if (packet.getVariableHeader().properties.tryGetProperty(
            PropertyType::TOPIC_ALIAS_MAXIMUM, serverTopicAliasMax)) {
    ...
    if (packet.getVariableHeader().properties.tryGetProperty(
            PropertyType::RETAIN_AVAILABLE, retainAvailable)) {
    ...
    if (packet.getVariableHeader().properties.tryGetProperty(
            PropertyType::MAXIMUM_PACKET_SIZE, maxPacketSize)) {
      m_connectionInfo.maxServerPacketSize =
    ...
    if (packet.getVariableHeader().properties.tryGetProperty(
            PropertyType::RECEIVE_MAXIMUM, serverReceiveMaximum)) {
    ...
    // Set-up receive maximum tracker with limits for broker and client.
    {
      m_receiveMaximumTracker =
          ReceiveMaximumTracker{m_connectionInfo.receiveMaximumAsClient,
                                m_connectionInfo.receiveMaximumAsServer};
    ...
    if (packet.getVariableHeader().properties.tryGetProperty(
            PropertyType::ASSIGNED_CLIENT_IDENTIFIER, assignedClientId)) {
      m_connectionInfo.connectArgs.clientId = assignedClientId->getString();
      LogInfo("MqttClient", "Broker assigned client identifier: %s",
              m_connectionInfo.connectArgs.clientId.c_str());
    }
    ...
    if (packet.getVariableHeader().properties.tryGetProperty(
            ...
            subIdentifierAvailable)) {
      m_connectionInfo.subscribeIdentifiersSupported =
          *subIdentifierAvailable == 1;
    }

    const SessionState prevSessionState{
        std::move(m_connectionInfo.sessionState)};
    m_connectionInfo.sessionState =
        SessionState{m_connectionInfo.connectArgs.clientId.c_str(),
                     m_connectionInfo.connectArgs.sessionExpiryInterval,
                     m_config.retryPublishIntervalMS};

    // If session present flag is set, merge previous session state into new one
    // (e.g. pending publish, pubrec, pubrel...).
    if (packet.getVariableHeader().flags.getFlagValue(
            ConnectAcknowledgeFlags::SESSION_PRESENT) == 1) {
      if (prevSessionState.getClientId().empty()) {
        LogError("MqttClient",
                 "Broker indicated existing session present, but client has no "
                 "previous session state to restore.");

        if (m_connectionStatus == ConnectionStatus::RECONNECTING) {
          handleFailedReconnect(
              std::move(packet),
              ClientErrorCode::Server_Session_Present_Mismatch);
        } else {
      ...
      m_connectionInfo.sessionState.addPrevSessionState(
          prevSessionState); // Merge previous session state into new one.
    }

    // TODO read mqtt5 spec for rest of properties of Connect Ack (relating to
    // publishing, subscribing and reconnection).

    if (m_connectionStatus == ConnectionStatus::RECONNECTING) {
      LogDebug(
          "MqttClient", "Reconnection complete to broker: %s Port: %s",
          m_connectionInfo.reconnectAddress.primaryAddress.hostname().c_str(),
          m_connectionInfo.reconnectAddress.primaryAddress.port().c_str());

      DISPATCH_EVENT_TO_CONSUMER([&, p = std::move(packet)]() {
        m_reconnectEvent(
            {ReconnectionStatus::SUCCEEDED, true, 0, ClientErrorCode::No_Error},
            p);
      });
    } else {
      LogDebug(
          "MqttClient", "Client connected to broker: %s Port: %s",
          m_connectionInfo.connectAddress.primaryAddress.hostname().c_str(),
          m_connectionInfo.reconnectAddress.primaryAddress.port().c_str());

      DISPATCH_EVENT_TO_CONSUMER([&, p = std::move(packet)]() {
        m_connectEvent({true, true, ClientErrorCode::No_Error}, p);
      });
    }
  } else {
    assert(m_connectionStatus == ConnectionStatus::RECONNECTING ||
           m_connectionStatus == ConnectionStatus::CONNECTING);
...
void MqttClientImpl::handleReceivedPublishAck(PublishAck &&packet) {
  ...
  DISPATCH_EVENT_TO_CONSUMER([&,
                              rsnCode = static_cast<std::uint8_t>(
                                  packet.getVariableHeader().reasonCode),
                              id = packetId]() {
    m_pubCompletedEvent({PacketType::PUBLISH_ACKNOWLEDGE, id, rsnCode});
  });
}
...
void MqttClientImpl::handleReceivedPublishComp(PublishComp &&packet) {
  ...
  DISPATCH_EVENT_TO_CONSUMER([&,
                              rsnCode = static_cast<std::uint8_t>(
                                  packet.getVariableHeader().reasonCode),
                              id = packetId]() {
    m_pubCompletedEvent({PacketType::PUBLISH_COMPLETE, id, rsnCode});
  });
}
...
void MqttClientImpl::handleReceivedPublishRec(PublishRec &&packet) {
  ...
  if (reasonCode >= PubRecReasonCode::UNSPECIFIED_ERROR) {
    LogWarning("MqttClient",
               "Received PUBREC packet with non-success reason code: %d for "
               "Packet ID %d. Cancelling publish message.",
               static_cast<std::uint8_t>(reasonCode), packetId);
  ...
  DISPATCH_EVENT_TO_CONSUMER([&,
                              rsnCode = static_cast<std::uint8_t>(
                                  packet.getVariableHeader().reasonCode),
                              id = packetId]() {
    m_pubCompletedEvent({PacketType::PUBLISH_RECEIVED, id, rsnCode});
  });
}
...
void MqttClientImpl::handleReceivedPublishRel(PublishRel &&packet) {
  ...
  DISPATCH_EVENT_TO_CONSUMER([&,
                              rsnCode = static_cast<std::uint8_t>(
                                  packet.getVariableHeader().reasonCode),
                              id = packetId]() {
    m_pubCompletedEvent({PacketType::PUBLISH_RELEASED, id, rsnCode});
  });
}
...
void MqttClientImpl::handleReceivedSubscribeAcknowledge(SubscribeAck &&packet) {
  ...
  for (auto iter = m_connectionInfo.pendingSubscriptions.begin();
       iter != m_connectionInfo.pendingSubscriptions.end(); ++iter) {
  ...
  if (!foundPacketId) {
    m_packetIdPool.releaseId(packetId);
    LogWarning("MqttClient",
               "Received SUBACK packet with Packet ID %d, but no pending "
               "subscription found for it.",
               packetId);
    return;
  }
  ...
  DISPATCH_EVENT_TO_CONSUMER(
      [&, id = packetId, res = std::move(results), p = std::move(packet)]() {
        m_subAckEvent({id, std::move(res)}, p);
      });
}

void MqttClientImpl::handleReceivedUnSubscribeAcknowledge(
  ...
  for (auto iter = m_connectionInfo.pendingUnSubscriptions.begin();
       iter != m_connectionInfo.pendingUnSubscriptions.end(); ++iter) {
    if (iter->packetId == packetId) {
      foundPacketId = true;
  ...
  if (!foundPacketId) {
    m_packetIdPool.releaseId(packetId);
    LogWarning("MqttClient",
               "Received UNSUBACK packet with Packet ID %d, but no pending "
               "un-subscription found for it.",
               packetId);
    return;
  }
  ...
  DISPATCH_EVENT_TO_CONSUMER(
      [&, id = packetId, res = std::move(results), p = std::move(packet)]() {
        m_unSubAckEvent({id, std::move(res)}, p);
      });
}
...
void MqttClientImpl::firePublishReceivedEvent(Publish &&packet) noexcept {
  ...
  if (properties->tryGetProperty<std::uint16_t>(PropertyType::TOPIC_ALIAS,
                                                topicAlias)) {
    if (m_connectionInfo.connectArgs.maximumTopicAliases == 0) {
      LogError("MqttClient",
               "Client does not allow topic aliases but server has sent us a "
               "topic alias of value %d.",
               *topicAlias);

      DisconnectArgs args;
      args.gracefulDisconnect = false;
      args.disconnectReasonText = "Client does not allow topic aliases but "
                                  "server has sent us a topic alias";
      const ClientError error{ClientErrorCode::Recv_Topic_Alias_Disallowed,
                              args.disconnectReasonText.c_str()};

      DISPATCH_EVENT_TO_CONSUMER([this, error]() { m_errorEvent(error, {}); });
      handleInternalDisconnect(DisconnectReasonCode::PROTOCOL_ERROR,
                               std::move(args));
      return;
    }

    if (*topicAlias <= 0 ||
        *topicAlias > m_connectionInfo.connectArgs.maximumTopicAliases) {
      LogError("MqttClient",
               "Publish packet TOPIC ALIAS is invalid: %d\nTOPIC ALIAS - Min "
               "allowed value: %d. Max allowed value: %d",
               *topicAlias, 1,
               m_connectionInfo.connectArgs.maximumTopicAliases);

      DisconnectArgs args;
      args.gracefulDisconnect = false;
      args.disconnectReasonText =
          "Publish packet TOPIC ALIAS value is invalid.";
      const ClientError error{ClientErrorCode::Recv_Invalid_Topic_Alias,
                              args.disconnectReasonText.c_str()};

      DISPATCH_EVENT_TO_CONSUMER([this, error]() { m_errorEvent(error, {}); });
      handleInternalDisconnect(DisconnectReasonCode::TOPIC_ALIAS_INVALID,
                               std::move(args));
      return;
    }
    ...
    if (topicName.empty()) {
      ...
      if (m_connectionInfo.topicAliases.tryFindTopicName(*topicAlias,
                                                         topicNameFound)) {
        topicName = topicNameFound;
      }
    } else {
      m_connectionInfo.topicAliases.tryAddTopicAlias(topicName.c_str(),
                                                     *topicAlias);
    }
  } else if (topicName.empty()) {
    LogError("MqttClient",
             "Publish packet TOPIC ALIAS is invalid and TOPIC NAME is empty.");
    ...
    const ClientError error{
        ClientErrorCode::Recv_Empty_Topic_Name_And_Invalid_Alias,
        args.disconnectReasonText.c_str()};
  ...
  // After packet is sent to application layer, send PUBACK packet back to the
  // server (As per QOS 1 specification)
  if (qos == Qos::QOS_1) {
  ...
  if (m_connectionStatus == ConnectionStatus::CONNECTED) {
    ...
    if (m_connectionInfo.awaitingPingResponse) {
      ...
      if (elapsedTimeSincePingReq > m_config.pingTimeOutMS) {
        ...
        args.disconnectReasonText =
            "Failed ping: Did not receive PingResp packet.";
...
void MqttClientImpl::tickSendPackets() {
  ...
  // error and start disconnect.
  if (!m_batchResultData.isRecoverable) {
    DisconnectArgs args;
    args.gracefulDisconnect = false;
    args.disconnectReasonText =
        "Hit unrecoverable error in sending packet: " +
        m_batchResultData.unrecoverableReasonStr + ", Encode result: " +
        m_batchResultData.lastSendResult.encodeResult.reason;
    args.clearQueue = true;
    const ClientError error{ClientErrorCode::Failed_Sending_Packet,
                            args.disconnectReasonText.c_str()};

    m_clientOptions.getCallbackDispatcher()->dispatch(
        [this, error, res = m_batchResultData.lastSendResult]() mutable {
          m_errorEvent(error, std::move(res));
        });
    handleInternalDisconnect(DisconnectReasonCode::UNSPECIFIED_ERROR, args);
  }
}
...
void MqttClientImpl::tickPendingPublishMessageRetries() {
  ...
  for (const auto &msg : msgs) {
    if (msg.nextRetryTime < std::chrono::steady_clock::now()) {
      ...
      LogTrace("MqttClient",
               "Retrying message: Type: %s, Packet ID: %d, Topic: %s",
               packetTypeToString(type), msg.data.packetID,
      ...
      break; // Messages are sorted in a way that the next retry time in message
             // list will be larger, so if we hit one that is not ready to be
             // retried, we can stop checking the rest.
    }
  }
}
...
void MqttClientImpl::handleFailedReconnect(ConnectAck &&packet,
  ...
  LogDebug("MqttClient", "Client could not reconnect. ConnectReasonCode: %d",
           static_cast<std::uint8_t>(packet.getVariableHeader().reasonCode));
  m_socket->close();
  m_receiveQueue.clear();

  if (errorCode == ClientErrorCode::No_Error &&
      m_connectionInfo.reconnectAddress.tryCycleToNextPrimaryAddress()) {
    reconnect();
  } else {
    m_sendQueue.clearQueue();
    ...
    DISPATCH_EVENT_TO_CONSUMER([&, p = std::move(packet),
      ...
      m_reconnectEvent({ReconnectionStatus::FAILED, false, 0, failedError}, p);
    });

    if (m_connectionInfo.hasBeenConnected) {
      DISPATCH_EVENT_TO_CONSUMER([&, failedError = errorCode]() {
        m_disconnectEvent({DisconnectReasonCode::NORMAL_DISCONNECTION, false,
                           false, failedError});
      });
    }
  }
...
void MqttClientImpl::handleFailedConnect(ConnectAck &&packet,
  ...
  m_connectionStatus = ConnectionStatus::DISCONNECTED;
  LogDebug("MqttClient",
           "Client could not connect to broker. ConnectReasonCode: %d",
           static_cast<std::uint8_t>(packet.getVariableHeader().reasonCode));
  ...
  DISPATCH_EVENT_TO_CONSUMER(
      [&, p = std::move(packet), failedError = errorCode]() {
        m_connectEvent({false, true, failedError}, p);
      });
}
...
void MqttClientImpl::handleTimeOutConnect() {
  m_connectionStatus = ConnectionStatus::DISCONNECTED;
  LogDebug("MqttClient",
           "Client could not connect to broker. Connection timed out");
  ...
  DISPATCH_EVENT_TO_CONSUMER(
      [&]() { m_connectEvent({false, false, ClientErrorCode::TimeOut}, {}); });
}
...
void MqttClientImpl::handleTimeOutReconnect() {
  ...
  if (m_connectionInfo.reconnectAddress.tryCycleToNextPrimaryAddress()) {
    ...
    DISPATCH_EVENT_TO_CONSUMER([&]() {
      m_reconnectEvent(
          {ReconnectionStatus::FAILED, false, 0, ClientErrorCode::No_Error},
          {});
    });
...
void MqttClientImpl::handleDecodeError(const DecodeResult &result) noexcept {
  ...
  LogInfo("MqttClient",
          "Failed to decode received packet. Packet Type: %d, Reason: %s",
          static_cast<std::uint8_t>(result.packetType), result.reason.c_str());

  if (m_connectionStatus == ConnectionStatus::CONNECTING) {
    const bool hasAck{result.packetType == PacketType::CONNECT_ACKNOWLEDGE};

    DISPATCH_EVENT_TO_CONSUMER([&, hA = hasAck, p = ConnectAck{}]() {
      m_connectEvent({false, hA, ClientErrorCode::Failed_Decoding_Packet}, p);
    });
  }
  const ClientError error{ClientErrorCode::Failed_Decoding_Packet,
                          args.disconnectReasonText.c_str()};
...
int MqttClientImpl::sendPacket(const BasePacket &packet) {
  if (m_socket == nullptr) {
    LogError("MqttClient",
             "Cannot send packet, socket is nullptr. Packet Type: %d",
             static_cast<std::uint8_t>(packet.getPacketType()));
    return -1;
  }

  LogInfo("MqttClient", "Sending packet. PacketType: %s, Size: %d",
          packetTypeToString(packet.getPacketType()),
          packet.getFixedHeader().getEncodedBytesSize() +
              packet.getFixedHeader().remainingLength.uint32Value());

  const ByteBuffer &bufferRef{packet.getDataBuffer()};

  if (static_cast<std::size_t>(m_socket->send(bufferRef)) == bufferRef.size()) {
    LogTrace("MqttClient", "Packet sent, Bytes: %s.",
             bufferRef.toString().c_str());
    return 0;
  }

  LogError("MqttClient", "Sending packet failed at socket level: %d",
           m_socket->getLastError());
