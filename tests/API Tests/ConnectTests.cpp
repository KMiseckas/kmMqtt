...
TEST_SUITE("MqttClient Connect") {
  ...
  TEST_CASE("Empty Hostname") {
    ...
    testContext.tryConnect(ClientErrorCode::Missing_Argument,
                           testContext.getDefaultConnectArgs(),
                           std::move(address));
  }
  ...
  TEST_CASE("Connect Event Success") {
    ...
    testContext.client->onConnectEvent().add(
        [&](const mqtt::ConnectEventDetails &details,
            const mqtt::ConnectAck &) {
  ...
  TEST_CASE("Connect Event  - Missing property length") {
    ...
    testContext.client->onErrorEvent().add(
        [&](ClientError error, mqtt::SendResultData data) {
          cError = error;
          rData = data;
        });
    ...
    CHECK(cError.errorMsg.empty() == false);
  }
  ...
  TEST_CASE("Connect Event - ConnectAck with error reason code triggers "
            "connect event with failure") {
    TestClientContext testContext{{}, true};
    ...
    testContext.client->onConnectEvent().add(
        [&](const mqtt::ConnectEventDetails &details,
            const mqtt::ConnectAck &packet) {
          connectEventFired = true;
          connectEventSuccess = details.isSuccessful;
          connectEventError = details.error;
          hasReceivedConnectAck = details.hasReceivedAck;

          CHECK(packet.getVariableHeader().reasonCode ==
                ConnectReasonCode::UNSPECIFIED_ERROR);
        });

    testContext.tryConnect();

    // Simulate ConnectAck with error reason code (e.g. 0x80 =
    // UNSPECIFIED_ERROR)
    ByteBuffer ackBuffer(5);
    ackBuffer += 32;   // Type
    ackBuffer += 3;    // Remaining Length
  ...
  TEST_CASE(
      "Connect Event - ConnectAck with missing bytes after remaining length") {
    TestClientContext testContext{{}, true};
  ...
  TEST_CASE("Connect Event - ConnectAck with missing bytes but receive "
            "remaining after") {
    TestClientContext testContext{{}, true};
  ...
  TEST_CASE(
      "Connect Event - ConnectAck with property length but no properties") {
    TestClientContext testContext{{}, true};
    ...
    ackBuffer += 0;  // Reason
    ackBuffer +=
        1; // Property Length (should be 1, but no property bytes follow)
    // No property bytes
  ...
  TEST_CASE("ConnectAck server keepAlive property is converted to milliseconds "
            "once") {
    TestClientContext testContext{{}, true};
    testContext.tryConnect();
  ...
  TEST_CASE("ConnectAck - Success with properties") {
    ...
    testContext.client->onConnectEvent().add(
        [&](const mqtt::ConnectEventDetails &details,
            const mqtt::ConnectAck &ack) {
          connectEventFired = true;
          CHECK(details.isSuccessful);
          CHECK(details.hasReceivedAck);
          const uint32_t *interval;
          CHECK(ack.getVariableHeader().properties.tryGetProperty(
              mqtt::PropertyType::SESSION_EXPIRY_INTERVAL, interval));
          CHECK(*interval == 1);
        });

    testContext.tryConnect();

    // Simulate ConnectAck with property length and a dummy property (property
    // id 0x11, value 0x01)
    ...
    ackBuffer +=
        0x11; // Property identifier (Session Expiry Interval for example)
  ...
  TEST_CASE("ConnectAck - Failure due to protocol error - disconnect called "
            "internally") {
    TestClientContext testContext{{}, true};
    bool connectEventFired = false;
    ...
    testContext.client->onDisconnectEvent().add(
        [&](const mqtt::DisconnectEventDetails &details) {
          disconnectCalled = true;
          CHECK(details.error.errorCode == mqtt::ClientErrorCode::No_Error);
          CHECK(details.reasonCode ==
                mqtt::DisconnectReasonCode::MALFORMED_PACKET);
    ...
    // Simulate ConnectAck with property length and a dummy property (property
    // id 0x11, value 0x01) but malformed packet
    ByteBuffer ackBuffer(10);
    ackBuffer += 32; // Type
    ackBuffer +=
        5; // Remaining Length (Protocol error here because 5 (Remaining Length)
           // + 1(Fixed Header) does not equal 10(Total size) in the ackBuffer)
    ackBuffer += 0; // Flag
    ackBuffer += 0; // Reason
  ...
  TEST_CASE("ConnectAck - Not authorized reason code") {
    ...
    testContext.client->onConnectEvent().add(
        [&](const mqtt::ConnectEventDetails &details,
          ...
          CHECK(packet.getVariableHeader().reasonCode ==
                mqtt::ConnectReasonCode::NOT_AUTHORIZED);
        });
  ...
  TEST_CASE("ConnectAck - Protocol error reason code") {
    ...
    testContext.client->onConnectEvent().add(
        [&](const mqtt::ConnectEventDetails &details,
          ...
          CHECK(!details.isSuccessful);
          CHECK(packet.getVariableHeader().reasonCode ==
                mqtt::ConnectReasonCode::PROTOCOL_ERROR);
        });
  ...
  TEST_CASE("Reconnect callback failure keeps reconnecting flow active") {
    ...
    address.otherAddresses.push_back(
        Address::createURL("", "localhost", "1884", ""));

    int reconnectingEventCount{0};
    bool connectFailureEventFired{false};

    testContext.client->onReconnectEvent().add(
        [&](const mqtt::ReconnectEventDetails &details,
            const mqtt::ConnectAck &) {
    ...
    testContext.client->onConnectEvent().add(
        [&](const mqtt::ConnectEventDetails &details,
            const mqtt::ConnectAck &) {
          if (!details.isSuccessful && !details.hasReceivedAck &&
              details.error == mqtt::ClientErrorCode::Socket_Connect_Failed) {
            connectFailureEventFired = true;
          }
        });

    testContext.tryConnect(mqtt::ClientErrorCode::No_Error,
                           TestClientContext::getDefaultConnectArgs(),
                           std::move(address));
    ...
    // Force the reconnect connect-attempt to fail; callback false is simulated
    // manually below.
    ...
    disconnectPacket += 2;
    disconnectPacket += static_cast<std::uint8_t>(
        mqtt::DisconnectReasonCode::USE_ANOTHER_SERVER);
    disconnectPacket += 0;
    testContext.receiveResponse(disconnectPacket);

    CHECK(testContext.client->getConnectionStatus() ==
          mqtt::ConnectionStatus::RECONNECTING);
